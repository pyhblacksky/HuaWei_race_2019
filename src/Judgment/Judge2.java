        ...
        for(; ;Time++) {
            ...
            while(isExistWaitingCar()){
                ...
                for(Cross cross : crossList){
                    //获取该路口的道路优先级
                    ArrayList<Road> priorityRoad = ThroughCrossRule2.RoadPriority(cross);//按照道路序号升序
        ...
        if (road.getEnd() == cross.getId()) {//此时说明是正向
            ...
            while(!changeRoad) {
                ...
                for (int lane = 0; lane < road.getLanes(); lane++) {
                    ...
                    boolean isUpdate = ThroughCrossRule2.Go_control_priority(topCar, cross, roadList, crossList);//是否可以更新道路矩阵
                    if (isUpdate) {   //可以更新矩阵
                        //反复调用
                        int pass_or_not = ThroughCrossRule2.Pass_or_Not(cross, lane, roadList, crossList);
                        int real_distance = ThroughCrossRule2.GO_distance(cross, lane, roadList, crossList, pass_or_not);
