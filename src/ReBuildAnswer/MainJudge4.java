...
public class MainJudge4 {
    ...
    public static ArrayList<Answer> buildAnswer(ArrayList<Car> cars, ArrayList<Cross> crosses,
        ...
        for(; ;Time++) {
            ...
            if(getInRoadCar().size() < limit){
                ...
                ArrayList<Car> realCarList = driveFirstCarInGarage(canRunCar, limit - getInRoadCar().size() , Time);
                //生成并记录answer
            ...
            /******************记录当前状态********************/
            ArrayList<Road> tempRoadList = buildTempRoadList();
            ArrayList<Car> tempCarList = buildTempCarList();
            ArrayList<Cross> tempCrossList = buildTempCrossList();
            rebuildState(tempRoadList, tempCarList, tempCrossList);//保持一一对应关系

            if(Time % 1 == 0){

                ArrayList<Road> forbidRoad = new ArrayList<>();
                ArrayList<Integer> forbidRoadId = new ArrayList<>();
                if(!PredictJudge4.judgeNoDeadLock(tempRoadList, tempCarList, tempCrossList, forbidRoadId)) {
                    for (int i : forbidRoadId) {
     ...
     * 记录临时roadList副本
     * */
    private static ArrayList<Road> buildTempRoadList(){
        ArrayList<Road> res = new ArrayList<>();
        for(Road road : roadList){
            res.add(new Road(road));
        }
        return res;
    }
     ...
     * 记录临时carList副本
     * */
    private static ArrayList<Car> buildTempCarList(){
     ...
     * 记录临时crossList副本
     * */
    private static ArrayList<Cross> buildTempCrossList(){
        ArrayList<Cross> res = new ArrayList<>();
        for(Cross cross : crossList){
            res.add(new Cross(cross));
        }
        return res;
    }
     ...
     * 判断死锁
     * */
    private static boolean JudgeDeadLock(ArrayList<Integer> forbidRoadIdList){
        //此时寻找满路，看是否可以形成环
        ArrayList<Road> fullRoads = countFullRoad();//获取满的路
        if(fullRoads.size() < 4){return false;}
        Set<List<Integer>> loop = FindLoop.findLoop(crossList, fullRoads);//根据满的路找环

        //if(fullRoads.size() > 4){
        //    return true;//满路大于4，直接返回true
        //} else {
        //    return false;
        //}
        ...
        if (loop.size() > 0) {
            ...
            return true;//有环，返回存在死锁，调整路线
        } else {
            return false;//无环，无锁，不调整
        }
    }

    /**
     * 计算当前道路剩余的空位
     * PorN:  1 表示正向，遍历S2E矩阵      2表示反向， 遍历E2S矩阵
     * */
    private static boolean countNowRoadBlankMoreTwo(Road road, int PorN){
        int count = 0;
        if(PorN == 1) {
            //S2E矩阵
    ...
    private static void reFindCarRoads(ArrayList<Car> inRoadCar, ArrayList<Road> forbidRoad, ArrayList<Answer> finalAnswer){
        for(Car car : inRoadCar){
            ...
            for(Road road : forbidRoad) {
                if(nextRoadList.contains(road)) {//接下来要行走的路包含这个列表，则进行相关处理
     ...
     * 状态重设, 保持一一对应关系
     * */
    private static void rebuildState(ArrayList<Road> tempRoadList, ArrayList<Car> tempCarList, ArrayList<Cross> tempCrossList){
        for(Car car : tempCarList){
            ArrayList<Road> getInRoadCarList = car.getRoads();
            ArrayList<Road> setInRoadCarList = new ArrayList<>();
            for(Road road : getInRoadCarList){
                setInRoadCarList.add(Util.getRoadFromId(road.getId(), tempRoadList));
            }
            car.setRoads(setInRoadCarList);
        }
        for(Cross cross : tempCrossList){
            Road upRoad = Util.getRoadFromId(cross.getUpRoad().getId(), tempRoadList);
            if(upRoad != null) {
                cross.setUpRoad(upRoad);
            }
            Road downRoad = Util.getRoadFromId(cross.getDownRoad().getId(), tempRoadList);
            if(downRoad != null) {
                cross.setDownRoad(downRoad);
            }
            Road leftRoad = Util.getRoadFromId(cross.getLeftRoad().getId(), tempRoadList);
            if(leftRoad != null) {
                cross.setLeftRoad(leftRoad);
            }
            Road rightRoad = Util.getRoadFromId(cross.getRightRoad().getId(), tempRoadList);
            if(rightRoad != null) {
                cross.setRightRoad(rightRoad);
            }
        }
     ...
     * 发出第一批车
     * */
    private static ArrayList<Car> driveFirstCarInGarage(ArrayList<Car> cars, int limitCount, int time){
        ...
        for (Car car : cars) {
            ...
            if (carStart == goCarRoad.getStart()) {
                //此时放入 S2E矩阵
                if(!countNowRoadBlankMoreTwo(goCarRoad, 1)){
                    continue;//剩余空位不大于2，不发这个车
                }
                int i = putLane(goCarRoad, 1);//表示车道
                int j = putLength(goCarRoad, car, 1);//表示放入长度
                if (i == -1 || j == -1) {//说明放不下,下一个车
                    continue;
                }
     ...
     * */
    private static ArrayList<Road> countFullRoad1() {
